...
OP_LIST=[lambda C_in,C_out, groups, stride: BnActConv2d(C_in,C_out, kernel_size=3, stride=stride, groups=groups),
         ...
         lambda C_in,C_out, groups, stride: BnActConv2d(C_in,C_out, kernel_size=5, stride=stride, groups=groups),
         lambda C_in,C_out, groups, stride: create_conv2d(C_in, C_out, kernel_size=3, stride=stride, groups=groups),
         lambda C_in,C_out, groups, stride: create_conv2d(C_in, C_out, kernel_size=1, stride=stride, groups=groups),
         lambda C_in,C_out, groups, stride: create_conv2d(C_in, C_out, kernel_size=5, stride=stride, groups=groups),
         lambda C_in,C_out, groups, stride: ActConv2dBn(C_in,C_out, kernel_size=3, stride=stride, groups=groups),
         lambda C_in,C_out, groups, stride: ActConv2dBn(C_in,C_out, kernel_size=1, stride=stride, groups=groups),
         lambda C_in,C_out, groups, stride: ActConv2dBn(C_in,C_out, kernel_size=5, stride=stride, groups=groups),
        ]
...
class ActConv2dBn(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size, stride, groups=1, norm_layer=BatchNormAct2d):
        super(ActConv2dBn, self).__init__()
        self.bn = norm_layer(out_chs, eps=0.001)
        self.conv = create_conv2d(in_chs, out_chs, kernel_size, stride=stride, groups=groups)
...
class DualPathBlock(nn.Module):
    def __init__(
            self, op_choices,in_chs, num_1x1_a, num_3x3_b, num_1x1_c, inc, groups, block_type='normal', b=False):
        ...
        self.c1x1_a = OP_LIST[op_choices[0]](in_chs, num_1x1_a, 1, 1)
        self.c3x3_b =  OP_LIST[op_choices[1]](num_1x1_a, num_3x3_b, 50, self.key_stride)
        if b:
            self.c1x1_c = CatBnAct(in_chs=num_3x3_b)
            self.c1x1_c1 = create_conv2d(num_3x3_b, num_1x1_c, kernel_size=1)
            self.c1x1_c2 = OP_LIST[op_choices[2]](num_3x3_b, inc, 1)
        else:
            self.c1x1_c = OP_LIST[op_choices[2]](num_3x3_b, num_1x1_c + inc, 1,1) #BnActConv2d(in_chs=num_3x3_b, out_chs=num_1x1_c + inc, kernel_size=1, stride=1)
            self.c1x1_c1 = None
            self.c1x1_c2 = None
    ...
    def forward(self, x) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        #print("Out 1x1",x_in.shape)
        x_in = self.c3x3_b(x_in)
        #print("Out 3x3",x_in.shape)
...
class DPN(nn.Module):
    def __init__(
            self, op_choices,small=False, num_init_features=64, k_r=96, groups=32, global_pool='avg',
...
def dpn107(pretrained=False, **kwargs):
    model_kwargs = dict(
        ...
        k_sec=(1,1,1,1), inc_sec=(20, 64, 64, 128), **kwargs)
    return _create_dpn('dpn107', pretrained=pretrained, **model_kwargs)


'''if __name__ == "__main__":
    network=DPN([0,4,2],num_init_features=128, k_r=200, groups=50, k_sec=(1,1,1,1), inc_sec=(20, 64, 64, 128))
    #network.to("cuda")
